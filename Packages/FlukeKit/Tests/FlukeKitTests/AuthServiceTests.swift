...
struct AuthServiceTests {
    ...
    func appleSignIn() async throws {
        let transport = RecordingTransport(
            status: 200,
            body: #"{"id":"observer-1","email":"cale@example.com","displayName":"Cale Lamb","role":"observer"}"#
        )
        let service = AuthService(api: APIClient(
    ...
    func accountDeletionRequiresNoContent() async throws {
        ...
        let acceptedService = AuthService(api: APIClient(
            baseURL: try #require(URL(string: "https://api.fluke.test")),
            transport: accepted
        ))
        ...
        let rejectedService = AuthService(api: APIClient(
            baseURL: try #require(URL(string: "https://api.fluke.test")),
