...
struct SubmissionServiceTests {
  ...
  func createsThenUploads() async throws {
    let transport = SubmissionTransport(responses: [
      .json(
        201,
        #"{"ok":true,"id":"s-1","identificationSuggestionId":null,"photoUploadToken":"token-1"}"#),
      .json(201, "{}"),
      .json(201, "{}"),
    ])
    ...
    let payload = try SubmissionValidator.validate(
      .fixture(
        photoCount: 2,
        ecotypeGuess: .biggs,
        localIdentification: LocalIdentificationSuggestion.fixture
      ))
    ...
    #expect(
      requests.dropFirst().allSatisfy {
        $0.value(forHTTPHeaderField: "x-photo-upload-token") == "token-1"
      })
    #expect(
      requests.first?.value(forHTTPHeaderField: "Idempotency-Key")
        == payload.clientSubmissionID.uuidString)
    let body = try #require(requests.first?.httpBody)
    let json = try #require(JSONSerialization.jsonObject(with: body) as? [String: Any])
    #expect(
      Set(json.keys) == [
        "clientSubmissionId", "observedAt", "latitude", "longitude", "locationName",
        "ecotypeGuess", "groupSize", "behaviorNotes", "observerEmail", "localIdentification",
      ])
    ...
    #expect(json["ecotypeGuess"] as? String == "BIGGS")
    let evidence = try #require(json["localIdentification"] as? [String: Any])
    #expect(evidence["scoreSemantics"] as? String == "uncalibrated_similarity_not_probability")
    #expect(json["photoCount"] == nil)
    #expect(json["existingReceipt"] == nil)
    #expect(
      requests[1].value(forHTTPHeaderField: "Idempotency-Key")
        == "\(payload.clientSubmissionID.uuidString):11111111-1111-1111-1111-111111111111")
    #expect(
  ...
  func partialRetryIsIdempotent() async throws {
    let firstTransport = SubmissionTransport(responses: [
      .json(
        201,
        #"{"ok":true,"id":"s-1","identificationSuggestionId":"suggestion-1","photoUploadToken":"token-1"}"#
      ),
      .json(201, "{}"),
    ...
    let photos = [ProcessedPhoto.fixture(1), .fixture(2)]
    let firstService = SubmissionService(
      api: APIClient(
    ...
    let retryService = SubmissionService(
      api: APIClient(
...
extension LocalIdentificationSuggestion {
  static var fixture: LocalIdentificationSuggestion {
...
extension ProcessedPhoto {
  static func fixture(_ byte: UInt8) -> ProcessedPhoto {
    ...
    return ProcessedPhoto(
      bytes: Data(repeating: byte, count: 32), fileName: "photo.jpg", idempotencyID: id)
  }
}
