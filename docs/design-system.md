...
| --- | --- | --- |
| `DateScrubberAtlas` | Release A | Accessible date control shared by Timeline and Trace |
...
Release A ships an opaque brand icon generated by [`scripts/generate-app-icon.swift`](../scripts/generate-app-icon.swift): the `abyss` dorsal fin on a `fog` background. The dorsal-fin path comes from the same `DorsalFinShape` SVG path the rest of the app uses, so there is one canonical glyph. Product review of the bundled artwork at App Store sizes remains part of the signed submission handoff.

Tinted-icon variants are not required for the Release A submission.
